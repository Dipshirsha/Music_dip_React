...
export default function HomeLangSong(props) {
  const a = useContext(DataContext);
  const [isDropdownOpen, setDropdownOpen] = useState(false);

  const handleClick = () => {
    // Filter out songs where the language equals props.lang
    const filteredSongs = props.song.filter(song => song.language.toLowerCase() === props.lang.toLowerCase());
  ...
  return (
    <div className="inline-block md:p-2 mr-2 cursor-pointer">
      {shouldRender && (
        props.data.language.toLowerCase() === props.lang.toLowerCase() && (
          <div className="relative group">
            ...
            <LazyLoadImage
              ...
              className="w-42 h-36 sm:w-50 wwwwwwwwwasm:h-55 md:w-42 md:h-36 lg:w-56 lg:h-48 rounded-md" // use normal <img> attributes as props
            />
